...
class Rebecca(plugins.Plugin):
    ...
    def on_loaded(self):
        logging.info("Rebecca: Plugin loaded  Setup begins...")
        self.faces_path = self.options.get('faces_path', '/c_rebecca')
        self.voice_path = self.options.get('voice_path', '/c_rebecca/voice.py')
    ...
    def _replace_voice_file(self):
        if os.path.exists(self.voice_path):
            try:
                ...
                logging.info(f"Rebecca: voice.py successfully replaced with {self.voice_path}")
            except Exception as e:
                logging.error(f"Rebecca: Error while replacing voice.py: {e}")
        else:
            logging.warning(f"Rebecca: Custom voice.py ({self.voice_path}) not found – no changes made.")

    def on_ui_setup(self, ui):
        import pwnagotchi.ui.faces as faces
        from pwnagotchi.ui import components

        logging.info("Rebecca: Loading PNG faces...")
        ...
        if os.path.isdir(self.faces_path):
            ...
            logging.info(f"Rebecca: {len(face_config) - 3} PNG faces loaded.")
        else:
            logging.warning(f"Rebecca: faces_path {self.faces_path} not found.")
                ...
                def draw_with_png(self, canvas, drawer):
                    if self.value and isinstance(self.value, str) and self.value.endswith('.png') and os.path.isfile(self.value):
                        try:
                            ...
                            logging.error(f"Rebecca PNG drawing: {e}")
                    original_draw(self, canvas, drawer)

                components.Text.draw = draw_with_png
                logging.info("Rebecca: PNG drawing enabled.")
        except Exception as e:
            logging.error(f"Rebecca: Error while patching Text.draw: {e}")
